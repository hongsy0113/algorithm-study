...
def solution(N, M, H, vertical_lines):
    ...
    for a in range(1, H + 1):
        for b in range(1, N):
            all_posible_lines.append((a, b))
    posible_lines = set(all_posible_lines) - vertical_lines

    # TODO 2: 두개 놓는 거 진행
    for case in combinations(list(posible_lines), 2):
        # line 유효한지 확인
        is_valid = True
        for line in case:
    ...
    # TODO 3 : 세 개 놓기
    for case in combinations(list(posible_lines), 3):
        # line 유효한지 확인
        is_valid = True
        for line in case:
            if not line_valid(line, vertical_lines):
